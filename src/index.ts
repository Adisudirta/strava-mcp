...
server.registerTool(
  "list_activities",
  {
    ...
    inputSchema: {
      ...
      before: z
        .number()
        .optional()
        .describe("Return only activities before this Unix timestamp"),
      after: z
        .number()
        .optional()
        .describe("Return only activities after this Unix timestamp"),
    },
  },
  async ({ per_page, page, before, after }) => {
    ...
    return ok(
      JSON.stringify(await stravaGet(`/athlete/activities?${params}`), null, 2),
    );
  },
);
...
server.registerTool(
  ...
  async ({ id }) =>
    ok(JSON.stringify(await stravaGet(`/activities/${id}`), null, 2)),
);
...
server.registerTool(
  ...
  {
    description:
      "Get the athlete's all-time and recent totals for runs, rides, and swims.",
  },
  async () => {
    const athlete = (await stravaGet("/athlete")) as { id: number };
    return ok(
      JSON.stringify(await stravaGet(`/athletes/${athlete.id}/stats`), null, 2),
    );
  },
);
